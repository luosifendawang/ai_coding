...
`gitplus commit` 会读取 Git Diff，执行安全扫描和脱敏，然后调用 AI Provider 生成 Commit Message 建议。
...
模型结果可以设置 `should_split=true` 并返回拆分建议。gitplus 只展示建议，不会修改暂存区，也不会自动拆分提交。
...
gitplus 支持纯 JSON、Markdown JSON 代码块、前后带说明文字的 JSON，并能修复简单尾随逗号。无法校验的响应会失败，不进入后续流程。

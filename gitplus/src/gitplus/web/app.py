"""FastAPI application factory for the local gitplus console."""
...
from gitplus.web.services.commit_web_service import CommitWebService
from gitplus.web.services.config_web_service import ConfigWebService
...
async def _repository_error(request: Request, exc: RepositoryWebError) -> Response:
    if hasattr(request.app.state, "audit_service"):
        request.app.state.audit_service.record(
            ...
            success=False,
            error_code=exc.code,
            session_id=request.cookies.get("gitplus_session"),
        )

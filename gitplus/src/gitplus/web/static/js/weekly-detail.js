(() => {
  ...
  async function load() {
    try {
      state.report = await gitplus.api(`/api/weekly/${encodeURIComponent(reportId)}`);
      state.removed = [];
      state.dirty = false;
      render();
      const warnings = JSON.parse(sessionStorage.getItem("gitplus-weekly-warnings") || "[]");
      sessionStorage.removeItem("gitplus-weekly-warnings");
      if (warnings.length) {
  ...
  async function action(name) {
    try {
      state.report = await gitplus.api(`/api/weekly/${encodeURIComponent(reportId)}/${name}`, {
        method: "POST",
        body: JSON.stringify({version: state.report.version}),
      });
      state.dirty = false;
      gitplus.toast(name === "confirm" ? "周报已确认" : "周报已重新打开", "success");
      render();
    } catch (error) {
      gitplus.toast(error.message, "error");
    }
  ...
  async function removeReport() {
    ...
    try {
      await gitplus.api(`/api/weekly/${encodeURIComponent(reportId)}?version=${state.report.version}&confirmed=true`, {method: "DELETE"});
  ...
  async function exportReport() {
    ...
    try {
      ...
      link.download = match ? match[1] : `gitplus-weekly.${format}`;
      link.click();

(() => {
  ...
  const load = async () => {
    try {
      const result = await gitplus.api(`/api/notifications/${encodeURIComponent(notificationId)}`);
  ...
  const retry = async () => {
    ...
    try {
      const result = await gitplus.api(`/api/notifications/${encodeURIComponent(notificationId)}/retry`, {
        method: "POST",
        body: JSON.stringify({
          confirmed: true,
      ...
      record = result.record;
      render();
      gitplus.toast(result.ok ? "重试发送成功。" : "重试失败，记录已更新。", result.ok ? "success" : "error");
    } catch (error) {
      gitplus.toast(error.message, "error");
    }
  };

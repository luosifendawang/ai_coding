(() => {
  ...
  async function preview() {
    setBusy(true);
    try {
      state.preview = await gitplus.api("/api/weekly/preview", {
        method: "POST",
  ...
  async function generate(event) {
    ...
    setBusy(true);
    try {
      const data = await gitplus.api("/api/weekly/generate", {
        method: "POST",
        body: JSON.stringify(payload),
      });
      sessionStorage.setItem("gitplus-weekly-warnings", JSON.stringify(data.warnings || []));
  ...
  async function loadReports() {
    ...
    try {
      const data = await gitplus.api(`/api/weekly${query}`);

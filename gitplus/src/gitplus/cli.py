"""Typer command line interface for gitplus."""
...
from gitplus.config import default_config, load_config
from gitplus.diagnostics.doctor import DoctorService
from gitplus.diagnostics.renderer import DiagnosticRenderer
from gitplus.exceptions import GitPlusError
from gitplus.logging import configure_logging
...
from gitplus.models.worklog import WorklogCreate, WorklogFilters, WorklogUpdate
from gitplus.release.readiness import ReleaseReadinessChecker
from gitplus.release.report import ReleaseReportRenderer
from gitplus.services.check_service import CheckResult, CheckService
...
def version_callback(value: bool) -> None:
    if value:
        console.print(f"gitplus {__version__}")
        raise typer.Exit()
...
def web(
    ...
    """启动仅监听本机的 Web 控制台。"""
    from gitplus.commands.web import run_web
...
def _print_commit_result(result, *, show_evidence: bool, show_security: bool, no_body: bool) -> None:
    console.print("[bold]gitplus Commit Message[/bold]")
    ...
    if generation.should_split:
        ...
        console.print("gitplus 只提供拆分建议，不会自动修改暂存区。")
...
def _print_check_table(
    ...
    *,
    ...
    console.print("[bold]gitplus 安全检查[/bold]")
...
def data_clear(
    ...
    """清理本地 gitplus 数据。"""
    if not yes and not typer.confirm("将清理 gitplus 本地数据库记录，是否继续？", default=False):
        console.print("已取消。")
        raise typer.Exit(code=1)

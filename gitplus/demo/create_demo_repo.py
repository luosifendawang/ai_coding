#!/usr/bin/env python3
"""Create a repeatable gitplus demo repository."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("/tmp/gitplus-demo"))
    args = parser.parse_args()
    repo = args.output.resolve()
    repo.mkdir(parents=True, exist_ok=True)
    (repo / ".gitplus-demo").write_text("demo\n", encoding="utf-8")
    if not (repo / ".git").exists():
        run(repo, "init")
    run(repo, "config", "user.name", "gitplus Demo")

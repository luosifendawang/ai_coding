...
from gitplus.exceptions import FeishuAuthenticationError, FeishuRequestError
...
def test_feishu_client_includes_api_error_details_in_send_failure() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/tenant_access_token/internal/"):
            return httpx.Response(
                200,
    ...
    with pytest.raises(
        FeishuRequestError, match="99991672.*Access denied.*log-id"
    ):
        client.send_text("hello")

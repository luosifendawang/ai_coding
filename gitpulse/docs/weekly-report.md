...
GitPulse weekly report generation turns local Git commits, confirmed CommitRecords, Worklogs, and user supplied notes into an editable developer weekly report.

## Command

```bash
gitpulse weekly --current --no-ai
gitpulse weekly --last-week --format markdown
gitpulse weekly --from 2026-07-27 --to 2026-08-02 --author you@example.com
gitpulse weekly --risk "发布前继续观察兼容性" --plan "补充飞书机器人发送"
gitpulse weekly --output weekly.md --save-draft
gitpulse weekly --confirm --json
```
...
`gitpulse weekly` is safe to run without network access. If no AI generator is configured, or `--no-ai` is used, GitPulse uses the deterministic rule-based generator.

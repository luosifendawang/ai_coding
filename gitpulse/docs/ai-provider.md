...
GitPulse 通过统一 `LLMProvider` 接口调用模型。业务层只依赖 `generate()`，不会直接调用 HTTP。
...
API Key 可以直接写在项目 `.gitpulse.yml` 的 `ai.api_key` 中，也可以通过环境变量读取。
...
本地 OpenAI 兼容服务可以使用 `localhost`、`127.0.0.1` 或显式配置 `is_local=true`。即使是本地模型，GitPulse 仍会先执行安全扫描，并优先使用脱敏 Diff。

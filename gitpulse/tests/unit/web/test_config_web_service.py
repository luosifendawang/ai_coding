...
from gitpulse.web.schemas.config import ConfigUpdateRequest, SecretUpdate
from gitpulse.web.services.config_web_service import (
    ConfigConflictError,
    ConfigWebService,
)
...
def test_effective_config_reports_environment_secret_without_value(
    ...
    current = service(tmp_path)
    monkeypatch.setenv("GITPULSE_API_KEY", "environment-secret-value")
...
def test_save_is_minimal_atomic_and_preserves_unknown_fields(tmp_path: Path) -> None:
    ...
    assert list(current.paths.project_config.parent.glob(".gitpulse.yml.bak.*"))

(() => {
  ...
  async function load() {
    ...
    try {
      const data = await GitPulse.api(`/api/worklogs?${query()}`);
  ...
  async function openExisting(id) {
    try {
      fill(await GitPulse.api(`/api/worklogs/${encodeURIComponent(id)}`), false);
    } catch (error) {
      GitPulse.toast(error.message, "error");
    }
  }
  ...
  async function copyExisting(id) {
    try {
      fill(await GitPulse.api(`/api/worklogs/${encodeURIComponent(id)}`), true);
    } catch (error) {
      GitPulse.toast(error.message, "error");
    }
  }
  ...
  async function save(event) {
    ...
    try {
      await GitPulse.api(id ? `/api/worklogs/${encodeURIComponent(id)}` : "/api/worklogs", {
        method: id ? "PUT" : "POST",
        body: JSON.stringify(body),
      });
      dialog.close();
      GitPulse.toast(id ? "工作日志已更新" : "工作日志已创建", "success");
      await load();
    } catch (error) {
  ...
  async function remove() {
    ...
    try {
      await GitPulse.api(`/api/worklogs/${encodeURIComponent(id)}?confirmed=true`, {method: "DELETE"});
      dialog.close();
      GitPulse.toast("工作日志已删除", "success");
      await load();
    } catch (error) {
      showError(error.message);
    }

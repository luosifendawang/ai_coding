(() => {
  ...
  const load = async () => {
    try {
      const [config, reports, records] = await Promise.all([
        GitPulse.api("/api/notifications/config-status"),
        GitPulse.api("/api/weekly?status=confirmed&page_size=50"),
        GitPulse.api("/api/notifications?page_size=12"),
      ]);
  ...
  const preview = async () => {
    if (!state.selectedReportId) {
      GitPulse.toast("请先选择周报。", "error");
      return;
    }
    try {
      const result = await GitPulse.api("/api/notifications/preview", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (result.duplicate && result.duplicate.is_duplicate) {
        GitPulse.toast(result.duplicate.message, "error");
      } else if (result.truncated) {
        GitPulse.toast("消息内容已自动压缩。", "info");
      }
    } catch (error) {
      GitPulse.toast(error.message, "error");
    }
  ...
  const send = async () => {
    ...
    if (!ok) return;
    try {
      const result = await GitPulse.api("/api/notifications/send", {
        method: "POST",
        body: JSON.stringify({
          report_id: state.selectedReportId,
          message_type: messageType.value,
      ...
      if (result.message) {
        GitPulse.toast(result.message, "error");
      } else {
        GitPulse.toast(result.ok ? "周报已发送。" : "发送失败，已记录。", result.ok ? "success" : "error");
      }
      await load();
    } catch (error) {
      GitPulse.toast(error.message, "error");
  ...
  const test = async () => {
    ...
    try {
      const result = await GitPulse.api("/api/notifications/test", {
        method: "POST",
        body: JSON.stringify({ confirmed: true }),
      });
      const success = result.response && result.response.success;
      GitPulse.toast(success ? "测试消息已发送。" : "测试失败。", success ? "success" : "error");
    } catch (error) {
      GitPulse.toast(error.message, "error");
    }
  };

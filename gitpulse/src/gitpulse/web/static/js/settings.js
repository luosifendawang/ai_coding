(() => {
  ...
  const load = async () => {
    try {
      const [config, sources] = await Promise.all([
        GitPulse.api("/api/config/effective"),
        GitPulse.api("/api/config/sources")
      ]);
      effective = config;
  ...
  const showValidation = (result) => {
    ...
    if (result.valid) {
      GitPulse.toast(result.warnings.length ? result.warnings.join(" ") : "配置校验通过", result.warnings.length ? "warning" : "success");
      return true;
    }
    ...
    GitPulse.toast("配置校验失败", "error");
    return false;
  };

  const preview = async () => {
    const result = await GitPulse.api("/api/config/preview", {method: "POST", body: JSON.stringify(payload())});
  ...
  document.getElementById("validate-config").addEventListener("click", async () => {
    try { showValidation(await GitPulse.api("/api/config/validate", {method: "POST", body: JSON.stringify(payload())})); }
    catch (error) { GitPulse.toast(error.message, "error"); }
  });
  previewButton.addEventListener("click", () => preview().catch((error) => GitPulse.toast(error.message, "error")));
  document.getElementById("confirm-save").addEventListener("click", async () => {
    try {
      effective = await GitPulse.api("/api/config", {method: "PUT", body: JSON.stringify(payload(true))});
      dialog.close();
      GitPulse.toast("配置保存成功，已重新加载", "success");
      await load();
    } catch (error) {
      GitPulse.toast(error.message, "error");
    }
  ...
  document.getElementById("restore-backup").addEventListener("click", async () => {
    ...
    try {
      await GitPulse.api(`/api/config/restore?scope=${scope.value}`, {method: "POST"});
      GitPulse.toast("备份恢复成功", "success");
      await load();
    } catch (error) { GitPulse.toast(error.message, "error"); }
  });
  [["test-ai", "test-ai"], ["test-feishu", "test-feishu"], ["test-storage", "test-storage"]].forEach(([id, endpoint]) => {
    document.getElementById(id).addEventListener("click", async () => {
      try {
        const result = await GitPulse.api(`/api/config/${endpoint}`, {method: "POST", body: JSON.stringify(payload())});
        GitPulse.toast(result.message, result.success ? "success" : "error");
      } catch (error) { GitPulse.toast(error.message, "error"); }
    });
  });

(() => {
  ...
  async function loadStatus({quiet = false} = {}) {
    ...
    try {
      ...
      state.status = await GitPulse.api("/api/repository/status");
  ...
  async function openDiff(path, source) {
    ...
    try {
      ...
      const result = await GitPulse.api(`/api/repository/diff?${params}`);
      if (result.binary) {
  ...
  async function mutate(action) {
    ...
    try {
      state.status = await GitPulse.api(`/api/repository/${action}`, {
        method: "POST",
      ...
      invalidateGenerated();
      renderStatus();
      GitPulse.toast(action === "stage" ? "文件已暂存" : "文件已取消暂存", "success");
    } catch (error) {
  ...
  async function scan() {
    ...
    try {
      state.scan = await GitPulse.api("/api/repository/security-scan", {
        method: "POST",
  ...
  async function generate() {
    ...
    try {
      state.generation = await GitPulse.api("/api/repository/commit/generate", {
        method: "POST",
        body: JSON.stringify({
          source: "staged",
      ...
      renderGeneration();
      GitPulse.toast("Commit Message 已生成", "success");
    } catch (error) {
      GitPulse.toast(error.message, "error");
    } finally {
      setBusy(false);
  ...
  async function commit() {
    ...
    try {
      const result = await GitPulse.api("/api/repository/commit", {
        method: "POST",
        body: JSON.stringify({
      ...
      invalidateGenerated();
      renderStatus();
      GitPulse.toast(`Commit ${result.commit.hash.slice(0, 8)} 创建成功，未执行 Push`, "success");
    } catch (error) {
      GitPulse.toast(error.message, "error");
    } finally {
      setBusy(false);
  ...
  window.addEventListener("focus", async () => {
    ...
    try {
      const result = await GitPulse.api(`/api/repository/revision?current=${encodeURIComponent(state.revision)}`);
      if (result.changed) {
        GitPulse.toast("仓库状态已变化，正在刷新", "warning");

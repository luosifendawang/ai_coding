...
from gitpulse.web.schemas.config import ConfigUpdateRequest, SecretUpdateAction
from gitpulse.web.services.config_backup_service import ConfigBackupService
from gitpulse.web.services.config_diff_service import ConfigDiffService
    ...
    def create(
        ...
        *,
        ...
        return cls(
            ...
            project_config=root / ".gitpulse.yml",
...
class ConfigWebService:
    """Share GitPulse config models while preserving scoped YAML overrides."""
    ...
    def get_effective_config(self) -> dict[str, object]:
        ...
        config = GitPulseConfig.model_validate(merged)
    ...
    def validate_update(self, request: ConfigUpdateRequest) -> dict[str, object]:
        try:
            ...
            config = GitPulseConfig.model_validate(effective)
    ...
    def config_for_request(self, request: ConfigUpdateRequest) -> GitPulseConfig:
        """Return validated effective config for an unsaved form request."""
        config = GitPulseConfig.model_validate(self._effective_for_request(request))
    ...
    def save_update(self, request: ConfigUpdateRequest) -> dict[str, object]:
        ...
        GitPulseConfig.model_validate(self._effective_for_request(request))
        ...
        try:
            ...
            raise
        keep = GitPulseConfig.model_validate(
    ...
    def _warnings(
        self, request: ConfigUpdateRequest, config: GitPulseConfig
    ) -> list[str]:
        warnings: list[str] = []
    ...
    def _validate_paths(self, config: GitPulseConfig) -> None:

...
from gitpulse.models.commit import CommitGenerationResult, CommitServiceResult
...
class CommitRecordService:
    ...
    def __init__(self, database: Database, id_generator: IdGenerator | None = None) -> None:
    ...
    def save_confirmed_record(self, request: CommitConfirmationRequest) -> CommitRecord:
        ...
        with UnitOfWork(self.database) as uow:
            ...
            record = CommitRecord(
                ...
                content_hash=self._content_hash(repository.id, request.final_subject, request.final_body, files),
                created_at=current,
                updated_at=current,
            )
    ...
    def _content_hash(self, repository_id: str, subject: str, body: list[str], files: list[str]) -> str:

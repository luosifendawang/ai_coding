...
class ReleaseReadinessChecker:
    ...
    def run(self) -> ReleaseReadiness:
        ...
        if not os.getenv("GITPULSE_RELEASE_CHECK_ACTIVE"):
            checks.insert(
                6,
                self._command(
                    "Pytest",
                    ...
                    timeout=300,
                    env={"GITPULSE_RELEASE_CHECK_ACTIVE": "1"},
                ),
            )
    ...
    def _package_resources(self) -> ReleaseCheck:
        required = [
            files("gitpulse.prompts").joinpath("commit_system.txt"),
            files("gitpulse.prompts").joinpath("weekly_system.txt"),
            files("gitpulse.web").joinpath("templates/base.html"),
            files("gitpulse.web").joinpath("templates/index.html"),
            files("gitpulse.web").joinpath("static/css/app.css"),
            files("gitpulse.web").joinpath("static/js/app.js"),
        ]
    ...
    def _build_and_install(self) -> ReleaseCheck:
        with tempfile.TemporaryDirectory(prefix="gitpulse-release-") as tmp:
            tmp_path = Path(tmp)
            build = self._run(
                [
                    sys.executable,
            ...
            cli = self._run(
                [
                    sys.executable,
                    "-c",
                    "from gitpulse.cli import main; import sys; sys.argv=['gitpulse','--version']; main()",
                ],
                timeout=20,
                env=env,
            )
    ...
    def _run(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="gitpulse-release-home-") as tmp:

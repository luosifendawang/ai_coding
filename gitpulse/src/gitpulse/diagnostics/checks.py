...
class DiagnosticChecks:
    ...
    def __init__(
        self, config: GitPulseConfig, *, database: Database | None = None
    ) -> None:
        self.config = config
    ...
    def user_config(self) -> DiagnosticItem:
        ...
        return DiagnosticItem(
            ...
            suggestion=None if path.exists() else "可运行 gitpulse setup --user 创建。",
        )

    def project_config(self) -> DiagnosticItem:
        path = Path.cwd() / ".gitpulse.yml"
        return DiagnosticItem(
            id="project_config",
            name="项目级配置",
            status=DiagnosticStatus.PASS if path.exists() else DiagnosticStatus.SKIPPED,
            message=str(path) if path.exists() else "当前项目未配置 .gitpulse.yml。",
        )
    ...
    def prompt_resources(self) -> DiagnosticItem:
        ...
        missing = [
            ...
            if not files("gitpulse.prompts").joinpath(name).is_file()
        ]
        if missing:
            return DiagnosticItem(
                id="prompts",
    ...
    def version(self) -> DiagnosticItem:
        return DiagnosticItem(
            ...
            name="版本",
            status=DiagnosticStatus.PASS,
            message=f"GitPulse {get_version()}",
        )
